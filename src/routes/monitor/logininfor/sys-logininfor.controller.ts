...
export class SysLogininforController {
  ...
  @ApiParam({ name: 'user_name', description: '用户名', example: 'admin' })
  @RequirePermissions('monitor:logininfor:unlock')
  @Get('unlock/:user_name')
  async unlock(@Param() params: UnlockLogininforParamDto) {
    const data = await this.sysLogininforService.unlock(params.user_name);
    return new ApiResponse(HttpStatus.OK, '操作成功', data);
  }
}
